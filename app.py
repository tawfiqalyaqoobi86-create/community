...
if menu == "📊 لوحة التحكم":
    ...
    if not df_p.empty:
        ...
        def make_ai_whatsapp_link(row):
            ...
            if phone and name:
                # صياغة الرسالة بالذكاء الاصطناعي المبسط - صياغة رسمية متوسطة الطول
                message = f"""سعادة الأستاذ {name} المحترم،،

السلام عليكم ورحمة الله وبركاته..
نتقدم لكم بخالص الشكر والتقدير على مساهماتكم القيمة وتفاعلكم المثمر في مجال ({p_type}). إننا نثمن عالياً دوركم الفعال كشريك استراتيجي في نجاح مبادراتنا المجتمعية، ونسأل الله لكم دوام التوفيق والسداد.
        ...
        if not df_p.empty:
            ...
            if st.button("توليد النص"):
                if doc_type == "دعوة شراكة":
                    st.info(f"""سعادة الأستاذ/ {p_name} المحترم،
تحية طيبة وبعد،،
بناءً على دوركم الفعال ومكانتكم المتميزة، يتشرف فريق تنمية العلاقات المجتمعية بدعوتكم للمساهمة في برامجنا ومبادراتنا القادمة. نحن نؤمن بأن خبراتكم ستشكل إضافة نوعية تساهم في تحقيق أهدافنا المشتركة لخدمة المجتمع.
تفضلوا بقبول فائق الاحترام والتقدير.""")
                elif doc_type == "خطاب شكر":
                    st.success(f"""سعادة الأستاذ/ {p_name} المحترم،
السلام عليكم ورحمة الله وبركاته،،
يتقدم فريق تنمية العلاقات المجتمعية بخالص الشكر والتقدير لسعادتكم على جهودكم الملموسة ومساهماتكم القيمة التي كان لها الأثر الطيب في نجاح برامجنا. إننا نثمن عالياً هذا العطاء ونتطلع دائماً لاستمرار هذا التعاون المثمر.
مع خالص تمنياتنا لكم بموفور الصحة والسداد.""")
                elif doc_type == "تقرير تعاون":
                    st.info(f"""سعادة الأستاذ/ {p_name} المحترم،
نرفق لسعادتكم ملخصاً لنتائج التعاون المشترك خلال الفترة الماضية، حيث أظهرت المؤشرات فاعلية كبيرة وتأثيراً إيجابياً ملموساً في كافة المجالات المستهدفة. نشكر لكم احترافيتكم العالية والتزامكم الدائم بتقديم الأفضل.
